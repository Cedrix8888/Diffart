...
class AIResult(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), index=True)  # 生成时间
...
class Layer(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), index=True)  # 图层创建时间
